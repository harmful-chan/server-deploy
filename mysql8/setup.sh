...
if istrue MYSQL8_INSTALL_PKG; then
    ...
    echo `check tar.zx $MYSQL8_NAME $MYSQL8_NAME.tar.zx https://dev.mysql.com/get/Downloads/MySQL-8.0/$MYSQL8_NAME.tar.xz`
...
if istrue MYSQL8_INIT_ROOT; then
    ...
    # 设置 'root'@'%' 密码 123456
    /usr/local/mysql8/bin/mysql --defaults-file=$(pwd)/$D/conf/my.cnf -uroot  mysql \
        -e "ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY '123456';"
    # 添加 'root'@'%' 权限
    /usr/local/mysql8/bin/mysql --defaults-file=$(pwd)/$D/conf/my.cnf -uroot -p123456 mysql \
...
if istrue MYSQL8_LOAD_SO; then
    $S ln -s /usr/local/mysql8/lib/libmysqlclient.so.21.2.29  /usr/lib/libmysqlclient.so.21
    $S ln -s /usr/local/mysql8/lib/libmysqlclient.so.21.2.29  /usr/lib/libmysqlclient.so
fi
